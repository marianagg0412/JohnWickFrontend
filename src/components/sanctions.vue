  ...
  <div class="wrapper">
    ...
    <div class="registration-box">
      ...
      <div class="dashboard">
        ...
        <button @click="setAction('getSanctionById')" class="dashboard-button">Get Sanction by ID</button>
        <button @click="setAction('updateSanction')" class="dashboard-button">Cambiar Sanction</button>
        <button @click="setAction('addSanctionToUser')" class="dashboard-button">Add sanction to user</button>
      </div>
    </div>
    <div class="modal" v-if="currentAction">
      <div class="scrollable-card">
        <button class="close-button" @click="currentAction = ''">X</button>
        <!-- Register Sanction Form -->
        <form v-if="currentAction === 'registerSanction'">
          <h3>Create Sanction</h3>
          <div class="input-box">
            <input type="text" placeholder="Enter cause" v-model="cause">
          </div>
          <div class="input-box button">
            <button @click.prevent="addCause">Add Cause</button>
          </div>
          ...
          <div class="input-box">
            <label for="sanctionType">Choose sanction type:</label>
            <select id="sanctionType" v-model="sanctionType">
              <option value="Multas">Multas</option>
              <option value="Excomunicacion">Excomunicacion</option>
              <option value="Muerte">Muerte</option>
            </select>
          </div>
          <div class="input-box button">
            <input type="submit" @click.prevent="createSanction" value="Submit">
          </div>
        </form>
        <!-- Fetched Sanctions -->
        <div v-if="currentAction === 'getSanctions'">
          <h3>Fetched Sanctions:</h3>
          <div v-if="sanctions.length">
            <ul>
              <li v-for="sanction in sanctions" :key="sanction._id">
                {{ formatSanction(sanction) }}
              </li>
            </ul>
          </div>
        </div>
        <!-- Get Sanction by ID Form -->
        <form v-if="currentAction === 'getSanctionById'">
          <h3>Get Sanction by ID</h3>
          <div class="input-box">
            <input type="text" placeholder="Enter ID" v-model="sanctionId">
          </div>
          <div class="input-box button">
            <input type="submit" @click.prevent="getSanctionById" value="Submit">
          </div>
          <div v-if="sanction">
            <h4>Fetched Sanction:</h4>
            <p>{{ formatSanction(sanction) }}</p>
          </div>
        </form>
        <!-- Update Sanction Form -->
        <form v-if="currentAction === 'updateSanction'">
          <h3>Modify Sanction</h3>
          <div class="input-box">
            <input type="text" placeholder="Enter ID of the sanction to modify" v-model="sanctionId">
          </div>
          <div class="input-box">
            <input type="text" placeholder="Enter new cause" v-model="newCause">
          </div>
          <div class="input-box button">
            <button @click.prevent="addNewCause">Add Cause</button>
          </div>
          <div v-if="newCauses.length">
            <h4>New Causes:</h4>
            <ul>
              <li v-for="cause in newCauses" :key="cause">{{ cause }}</li>
            </ul>
          </div>
          <div class="input-box button">
            <input type="submit" @click.prevent="updateSanction" value="Submit">
          </div>
        </form>
